...
$query = "
    SELECT 
        CASE 
            WHEN m.sender_id = $buyer_id THEN m.receiver_id 
            ELSE m.sender_id 
        ...
        (SELECT COUNT(*) FROM messages 
         WHERE receiver_id = $buyer_id 
           AND sender_id = CASE WHEN m.sender_id = $buyer_id THEN m.receiver_id ELSE m.sender_id END
           AND property_id = m.property_id 
           AND is_read = 0) as unread_count,
        (SELECT message FROM messages 
         WHERE (sender_id = $buyer_id AND receiver_id = CASE WHEN m.sender_id = $buyer_id THEN m.receiver_id ELSE m.sender_id END AND property_id = m.property_id)
            OR (sender_id = CASE WHEN m.sender_id = $buyer_id THEN m.receiver_id ELSE m.sender_id END AND receiver_id = $buyer_id AND property_id = m.property_id)
    ...
    FROM messages m
    JOIN users u ON u.id = CASE WHEN m.sender_id = $buyer_id THEN m.receiver_id ELSE m.sender_id END
    LEFT JOIN properties p ON p.id = m.property_id
    WHERE m.sender_id = $buyer_id OR m.receiver_id = $buyer_id
...
$selected_other_user = isset($_GET['user']) ? (int)$_GET['user'] : 0;
$selected_property_id = isset($_GET['property']) ? (int)$_GET['property'] : 0;
$messages_thread = [];

if ($selected_other_user && $selected_property_id) {
    $msg_query = "
        SELECT m.*, u.full_name as sender_name, u.profile_pic as sender_pic
        FROM messages m
        JOIN users u ON u.id = m.sender_id
        WHERE ((m.sender_id = $buyer_id AND m.receiver_id = $selected_other_user AND m.property_id = $selected_property_id)
           OR (m.sender_id = $selected_other_user AND m.receiver_id = $buyer_id AND m.property_id = $selected_property_id))
    ...
    mysqli_query($conn, "UPDATE messages SET is_read = 1 
                         WHERE sender_id = $selected_other_user 
                           AND receiver_id = $buyer_id 
                           AND property_id = $selected_property_id");
    ...
    $conv_query = "
        ...
        WHERE u.id = $selected_other_user
    ";
    $conv_result = mysqli_query($conn, $conv_query);
    if ($conv_result) {
        $selected_conversation = mysqli_fetch_assoc($conv_result);
        $unread_conv_query = "SELECT COUNT(*) as cnt FROM messages 
                              WHERE sender_id = $selected_other_user 
                                AND receiver_id = $buyer_id 
                                AND property_id = $selected_property_id 
                                AND is_read = 0";
        $unread_res = mysqli_query($conn, $unread_conv_query);
        if ($unread_res) {
            $selected_conversation['unread_count'] = (int)mysqli_fetch_assoc($unread_res)['cnt'];
        }
    }
}
...
<div class="buyer-dashboard-wrapper">
    ...
    <aside class="buyer-sidebar" id="buyerSidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li><a href="buyer-messages.php" class="active">
            ...
            <div class="buyer-container">
                ...
                <div class="messages-container">
                    ...
                    <div class="conversation-list">
                        <div class="header">
                            <h2>Messages</h2>  <!-- Changed from "Conversations" to "Messages" -->
                        </div>
                        <div class="search-box">
                            <input type="text" id="searchConv" placeholder="Search messages..." onkeyup="filterConversations()">  <!-- Changed placeholder -->
                        </div>
                        <div class="conversation-items" id="convList">
                            ...
                            <?php else: ?>
                                <?php foreach ($conversations as $conv):
                                    $is_active = ($selected_other_user == $conv['other_user_id'] && $selected_property_id == $conv['property_id']);
                                ...
                                <div class="conversation-item <?php echo $is_active ? 'active' : ''; ?>" 
                                     data-user="<?php echo $conv['other_user_id']; ?>" 
                                     data-property="<?php echo $conv['property_id']; ?>"
                                     onclick="selectConversation(<?php echo $conv['other_user_id']; ?>, <?php echo $conv['property_id']; ?>)">
                                    <div class="avatar">
                                        <?php if ($avatar): ?>
                    ...
                    <div class="conversation-view" id="conversationView">
                        <?php if ($selected_conversation && !empty($messages_thread)): ?>
                            ...
                            <div class="view-header">
                                ...
                                <div class="actions">
                                    <button onclick="deleteConversation(<?php echo $selected_other_user; ?>, <?php echo $selected_property_id; ?>)" title="Delete Conversation" class="delete-conv">
                            ...
                            <div class="messages-thread" id="messageThread">
                                <?php foreach ($messages_thread as $msg):
                                    $is_sent = ($msg['sender_id'] == $buyer_id);
...
function selectConversation(userId, propertyId) {
    window.location.href = 'buyer-messages.php?user=' + userId + '&property=' + propertyId;
}
...
function sendMessage() {
    ...
    .then(data => {
        if (data.success) {
            ...
            updateConversationLastMsg(otherUserId, propertyId, msg);
        } else {
...
function updateConversationLastMsg(userId, propertyId, msg) {
    const items = document.querySelectorAll('.conversation-item');
    items.forEach(item => {
        if (item.dataset.user == userId && item.dataset.property == propertyId) {
...
function deleteConversation(userId, propertyId) {
    deleteTarget = { user: userId, property: propertyId };
    ...
    document.getElementById('confirmDeleteBtn').onclick = function() {
        performDeleteConversation(userId, propertyId);
    };
    document.getElementById('deleteModal').classList.add('active');
}

function performDeleteConversation(userId, propertyId) {
    fetch('ajax-messages-buyer.php?delete_conversation=1&user=' + userId + '&property=' + propertyId, {
        headers: {
            'X-Requested-With': 'XMLHttpRequest'
        }
    })
    ...
    .then(data => {
        if (data.success) {
            ...
            window.location.href = 'buyer-messages.php';
        } else {
...
function deleteMessage(msgId) {
    ...
    fetch('ajax-messages-buyer.php?delete_message=1&msg_id=' + msgId, {
        headers: {
            'X-Requested-With': 'XMLHttpRequest'
        }
    })
...
function saveEditMessage(msgId, btn) {
    ...
    fetch('ajax-messages-buyer.php?edit_message=1&msg_id=' + msgId + '&message=' + encodeURIComponent(newText), {
        headers: {
            'X-Requested-With': 'XMLHttpRequest'
        }
    })
