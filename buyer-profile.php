...
// FETCH USER DATA
// ============================================================
$user_query = mysqli_query($conn, "SELECT * FROM users WHERE id = $buyer_id");
...
$res = mysqli_query($conn, "SELECT COUNT(*) as total FROM properties WHERE user_id = $buyer_id");
...
if ($res && mysqli_num_rows($res) > 0) {
    $res2 = mysqli_query($conn, "SELECT SUM(views) as total FROM properties WHERE user_id = $buyer_id");
...
if (isset($_FILES['profile_pic']) && $_FILES['profile_pic']['error'] == 0 && $_FILES['profile_pic']['size'] > 0) {
    ...
    if (in_array($ext, $allowed)) {
        ...
        $new_filename = 'user_' . $buyer_id . '_' . time() . '.' . $ext;
        $target_file = $upload_dir . $new_filename;

        $old_files = glob($upload_dir . 'user_' . $buyer_id . '_*');
        if ($old_files) {
        ...
        if (move_uploaded_file($_FILES['profile_pic']['tmp_name'], $target_file)) {
            if ($has_pic_column) {
                ...
                mysqli_query($conn, "UPDATE users SET profile_pic = '$pic_path' WHERE id = $buyer_id");
            }
            $_SESSION['profile_pic'] = $new_filename;
            $profile_pic_path = $target_file . '?t=' . time();
...
if (isset($_GET['delete_photo']) && $_GET['delete_photo'] == 1) {
    ...
    if ($has_pic && !empty($pic_to_delete)) {
        ...
        if ($has_pic_column) {
            mysqli_query($conn, "UPDATE users SET profile_pic = NULL WHERE id = $buyer_id");
        }
        ...
        header("Location: buyer-profile.php?deleted=1");
        exit();
    } else {
        header("Location: buyer-profile.php?error=nopic");
        exit();
    }
}
...
if (isset($_POST['update_profile'])) {
    $has_pic = !empty($profile_pic_path);
...
if (isset($_POST['change_password'])) {
    ...
    if (password_verify($current, $user['password'])) {
        if ($new == $confirm && strlen($new) >= 6) {
            ...
            mysqli_query($conn, "UPDATE users SET password = '$hashed' WHERE id = $buyer_id");
...
if (isset($_POST['delete_account'])) {
    ...
    mysqli_query($conn, "DELETE FROM users WHERE id = $buyer_id");
    session_destroy();
...
?>
<style>
    /* ===== BUYER DASHBOARD LAYOUT (identical to admin) ===== */
    ...
    /* ===== BUTTONS – FIX: width auto, inline-flex ===== */
    .btn {
        padding: 10px 22px;
        border-radius: 12px;
        font-size: 13px;
    ...
    /* Only used for full-width danger actions */
    .btn-block {
        width: 100%;
    }
...
<div class="buyer-dashboard-wrapper">
    ...
    <aside class="buyer-sidebar" id="buyerSidebar">
        ...
        <ul class="sidebar-menu">
            <li><a href="buyer-profile.php" class="active">
            ...
            <div class="profile-page-grid">
                ...
                <div class="profile-side-card">
                    ...
                    <div class="photo-actions">
                        ...
                        <?php if (!empty($profile_pic_path)): ?>
                            <a href="buyer-profile.php?delete_photo=1"
                    ...
                    <div class="bottom-settings-row">
                        ...
                        <div class="settings-card">
                            ...
                            <form method="POST">
                                ...
                                <!-- Removed btn-block, so this button fits text width -->
                        ...
                        <div class="settings-card">
                            ...
                            <div class="account-zone-notice">
                                ...
                                <div>
                                    ...
                                    <p>All your data, saved properties and messages will be removed.</p>
                                </div>
                            </div>
